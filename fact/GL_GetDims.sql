/*
drop procedure [dbo].GL_GetDims
Test >>> =============================================
exec [dbo].[GL_GetDims] 'IC Troika Dialog', 3, 14, 0, 'Nav-Offshores-Day'
exec [dbo].[GL_GetDims] 'ID Investments', 2, 37, 4, 'Nav-Offshores-Day'
Test <<< =============================================
*/
alter PROCEDURE [dbo].GL_GetDims
(
...
as begin
    ...
    --set @ReadTable  = '[dbo].[##FactGL_' +  @CompanyName + '_' + convert(varchar(3), @Partition) + '_B]'
    --set @ReadTable  = '[dbo].[#FactGL_' + convert(varchar(3), @Partition) + '_B]'
	...
	select
		...
		--,min(iif([Dimension Code] = ''CITY'', [Dimension Value Code], NULL)) as [CITY]
		--,min(iif([Dimension Code] = ''COUNTRY'', [Dimension Value Code], NULL)) as [COUNTRY]
		--,min(iif([Dimension Code] = ''LEDGER.ACC'', [Dimension Value Code], NULL)) as [LEDGER.ACC]
