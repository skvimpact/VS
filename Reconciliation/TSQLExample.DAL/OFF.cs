...
using TSQLExample.DAL.RF_Resources;
    ...
    public class OFF
    {
        ...
        static private ScriptExecutor off()
        {
            if (_off == null)
            {
               ...
               // string ddd = builder.ConnectionString;

                using (RFResourceControllerClient client = new RFResourceControllerClient())
                {
                    ConnectionStringSettings conn = client.GetConnectionStringSettings("MSSQL_NAVOFFSH");
                    _off = new ScriptExecutor(conn.ConnectionString, conn.ProviderName);
                }
        ...
        static public ICollection<CompanyAndConsts> getCompaniesAndConsts()
        {
            String[] consts = { "beautiful" , "wonderful", "as", "do" };
            /*
            ISet<CompanyAndConsts> companies = new SortedSet<CompanyAndConsts>();
            off().Run<CompanyAndConsts>(
                                TSQLScriptBuilder.getCompanyList(),
                                companies,
                                consts);
            return companies;*/
            ...
            //    return off().Run<CompanyAndConsts>(
            //                      TSQLScriptBuilder.getCompanyList(),
            //                    consts);
            return null;
        }
